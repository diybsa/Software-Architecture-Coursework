...
The business concept model captures the technology-free entities the system reasons about and how they relate.

**Entities:**
- **User** — initiates requests for share price data
- **Share** — a financial asset identified by a ticker symbol (e.g. AAPL, MSFT)
- **PriceData** — a single day's closing price for a share
- **MarketDataProvider** — the external source from which share prices are obtained

**Relationships:**
- A User *requests* data for one or more Shares
- A Share *contains* one or more PriceData records (composition — a PriceData entry has no meaning without its parent Share)
- The system *retrieves* Share data from a MarketDataProvider

These four entities and three relationships are the minimum set needed to express every functional requirement. Each later becomes either a domain class (`Share`, `PriceData`), a port interface (`MarketDataProvider`), or an external actor (`User`).
...
System interfaces decouple the application from the technology used for external data retrieval and persistence. Both are owned by the **application layer** (`com.Ga3.application.ports`), with concrete adapters living in the infrastructure layer.
...
#### ShareRepository
Responsible for storing and retrieving share data locally (supports offline operation per NFR1).
...
Locating these interfaces in the application layer — not in infrastructure — is what makes the architecture satisfy the **Dependency Inversion Principle**: the high-level application module defines what it needs; low-level infrastructure adapters implement it. The application layer never imports anything from `infrastructure`.
...
The system is organised into four Clean Architecture layers. Dependencies always point inward.

| Layer | Components | Responsibility |
|-------|-----------|----------------|
| Presentation | `ShareController`, `ShareRequestHandler` interface | Receives user requests; delegates to application layer |
| Application | `ShareService`, `GetShareDataUseCase`, `CompareTwoSharesUseCase`, ports (`MarketDataProvider`, `ShareRepository`) | Encodes business rules; owns abstractions of what infrastructure must provide |
| Domain | `Share`, `PriceData` | Core business entities; no external dependencies |
| Infrastructure | `MockMarketDataProvider`, `InMemoryShareRepository` | Concrete adapters that implement application ports |

**Interface Allocation:**

| Component | Implements (provided) | Depends on (required) |
|-----------|----------------------|----------------------|
| `ShareController` | `ShareRequestHandler` (1) | `ShareService` (1) |
| `ShareService` | own façade operations (1) | `GetShareDataUseCase`, `CompareTwoSharesUseCase` (2) |
| `GetShareDataUseCase` | `execute()` (1) | `MarketDataProvider`, `ShareRepository` (2) |
| `CompareTwoSharesUseCase` | `execute()` (1) | `GetShareDataUseCase` (1) |
| `MockMarketDataProvider` | `MarketDataProvider` (1) | — |
| `InMemoryShareRepository` | `ShareRepository` (1) | — |

`ShareService` and `GetShareDataUseCase` are the only components that depend on more than one interface — appropriate because they are coordinators. Every other component provides or depends on exactly one interface, keeping responsibilities narrow.

Dependencies flow `presentation` → `application` → `domain`, with `infrastructure` depending on `application` (it implements the ports defined there). The domain layer has no outward dependencies.

---

### 4.7 Business Interfaces and Collaboration

Where system interfaces live at the boundary with the external world, **business interfaces** expose the operations that the system's *business components* offer — derived directly from the use cases.

| Business Component | Business Interface | Key Operations | Use Case Source |
|--------------------|--------------------|----------------|------------------|
| Presentation | `ShareRequestHandler` | `requestShare(symbol, start, end)` | Get Share Data |
| Application (façade) | `ShareService` | `getShareData(...)`, `compareShares(...)` | Get Share Data, Compare Two Shares |
| Use Case | `GetShareDataUseCase`, `CompareTwoSharesUseCase` | `execute(...)` | One per use case |

Each business interface operation traces directly back to a step in a use case — no operation exists in the design that is not justified by the use case model.
...
The collaboration diagram shows how the business and system interfaces are invoked at runtime. Messages are numbered in execution order:

1. `:User` → `:ShareController` : `requestShare(symbol, start, end)`
2. `:ShareController` → `:ShareService` : `getShareData(symbol, start, end)`
3. `:ShareService` → `:GetShareDataUseCase` : `execute(symbol, start, end)`
4. `:GetShareDataUseCase` → `:GetShareDataUseCase` : `validate(input)` (date order and 2-year range)
5. `:GetShareDataUseCase` → `:MockMarketDataProvider` : `fetchShareData(symbol, start, end)`
6. `:GetShareDataUseCase` → `:InMemoryShareRepository` : `save(share)`

The same collaboration is shown in classical sequence form for reference:
...
Both views make the inward flow of control explicit: the User calls into Presentation, which calls into the Application façade, which delegates to a Use Case object, which finally invokes ports implemented by Infrastructure adapters.

---

### 4.8 Clean Architecture Implementation

The Java package structure directly reflects the Clean Architecture layers, with ports owned by the application layer and adapters in infrastructure:
...
com.Ga3
 ...
 │   │   └── CompareTwoSharesUseCase.java
 │   └── ShareService.java
 ├── infrastructure/
 │   ├── MockMarketDataProvider.java
 │   └── InMemoryShareRepository.java
 ├── presentation/
 │   ├── ShareRequestHandler.java
 │   └── ShareController.java
 └── Main.java
```

`GetShareDataUseCase` is the single source of truth for the Get Share Data business rules (symbol non-empty, dates supplied, start before end, range ≤ 2 years). `ShareService` and `ShareController` contain no business logic — they delegate. This prevents the same rule from drifting out of sync across multiple classes.

`Main.java` is the **composition root** — the only place where infrastructure adapters are instantiated. Everywhere else, code talks to a port, never a concrete class. Swapping the mock data provider for a real Yahoo Finance client would require changes only to this one block of wiring code.
